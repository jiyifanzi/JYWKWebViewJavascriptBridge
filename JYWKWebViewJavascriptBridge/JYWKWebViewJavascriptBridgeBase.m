//
//  JYWKWebViewJavascriptBridgeBase.m
//  FinderBox
//
...
@implementation JYWKWebViewJavascriptBridgeBase
...
- (void)flushMessageQueue:(NSString *)messageQueueString{
    ...
    for (JYWVJBMessage* message in messages) {
        if (![message isKindOfClass:[JYWVJBMessage class]]) {
        ...
        if (responseId) {
            Callback responseCallback = _responseCallbacks[responseId];
            ...
            if (callbackId) {
                responseCallback = ^(id responseData) {
                    ...
                    JYWVJBMessage* msg = @{ @"responseID":callbackId, @"responseData":responseData };
...
- (void)_queueMessage:(JYWVJBMessage*)message {
    if (self.startupMessageQueue) {
...
- (void)_dispatchMessage:(JYWVJBMessage*)message {
    ...
    messageJSON = [messageJSON stringByReplacingOccurrencesOfString:@"\u202C" withString:@"\\f"];
    messageJSON = [messageJSON stringByReplacingOccurrencesOfString:@"\u2028" withString:@"\\u2028"];
    messageJSON = [messageJSON stringByReplacingOccurrencesOfString:@"\u2029" withString:@"\\u2029"];
...
- (void)_log:(NSString *)action json:(id)json {
    ...
    if ([json length] > logMaxLength) {
        NSLog(@"JY-WVJB %@: %@ [...]", action, [json substringToIndex:logMaxLength]);
    } else {
        NSLog(@"JY-WVJB %@: %@", action, json);
    }
}
